...
class Bot(asynchat.async_chat): 
   ...
   def initiate_connect(self, host, port): 
      if self.verbose: 
         message = 'Making connection to %s:%s...' % (host, port)
   ...
   def handle_connect(self): 
      if self.verbose:
         print >> sys.stderr, 'Made connection.'
      if self.password: 
   ...
   def msg(self, recipient, text): 
      ...
      if messages.count(text) >= 5: 
         ...
         if messages.count('...') >= 1: # '...' old method of telling a user to stop spamming
            self.sending.release()
            return
   ...
   def error(self, origin): 
      ...
      except: self.msg(origin.sender, "Unknown source, getting an error!")
