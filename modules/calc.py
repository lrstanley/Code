...
def calc(code, input): 
    """Google calculator."""
    if not input.group(2): return code.reply(code.color('red', 'Nothing to calculate.'))
    q = input.group(2)
    if '=' in q and not q.endswith('='):
        return code.reply(code.color('red', 'I cannot solve equations.'))
    q = q.replace('x','*').replace('=','').strip().encode('utf-8')
    # utf-8 U+03D5, utf-8 U+03C0
    q = q.replace('\xcf\x95', 'phi').replace('\xcf\x80', 'pi')
