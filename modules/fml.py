...
def fml_random():
    """fml - Retrieve random FML's, via FMyLife.com's dev API."""
    try:
        r = urllib2.urlopen('http://api.fmylife.com/view/random/1%s?language=%s&key=%s' % (
            is_mature,
            language, key
        )).read()
    except:
    ...
    return {
        'fml': h.unescape(re.sub(r'\<.*?\>', '', fml[0]).strip()),
...
def fml_id_search(query_id):
    ...
    try:
        r = urllib2.urlopen('http://api.fmylife.com/view/%s/nocomment?language=%s&key=%s' % (
            str(query_id),
            language, key
        )).read()
    except:
