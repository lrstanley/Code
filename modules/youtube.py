...
def setup(code):
    ...
    if not code.memory.contains('url_exclude'):
        code.memory['url_exclude'] = [regex]
    else:
        exclude = code.memory['url_exclude']
        exclude.append(regex)
        code.memory['url_exclude'] = exclude

def ytget(code, input, uri):
    try: bytes = web.get(uri)
    except:
        code.say('Something went wrong when accessing the YouTube API.')
        return err
...
def ytsearch(code, input):
    ...
    uri = uri.replace(' ', '+')
    video_info = ytget(code, input, uri)

    if video_info is 'err':
        return

    if video_info['link'] == 'N/A':
        code.say("Sorry, I couldn't find the video you are looking for")
        return
